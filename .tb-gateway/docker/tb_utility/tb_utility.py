...
class TBUtility:
    ...
    def decode(message):
        try:
            if isinstance(message.payload, bytes):
                content = loads(message.payload.decode("latin-1"))
            else:
                content = loads(message.payload)
        except JSONDecodeError:
            try:
                content = message.payload.decode("latin-1")
            except JSONDecodeError:
    ...
    def topic_to_regex(topic):
        str = topic.replace("+", "[^/]+").replace("#", ".+")
        str = str[:-1].replace("$", "\$") + str[-1]
        return str

    @staticmethod
    def regex_to_topic(regex):
        str = regex.replace( "[^/]+", "+").replace(".+", "#")
        str = str[:-1].replace("\$", "$") + str[-1]
        return  str
